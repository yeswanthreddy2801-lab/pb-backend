"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.renewSubscription = exports.getSubscriptionById = exports.getMySubscriptions = exports.createSubscription = void 0;
const subscriptionsService = __importStar(require("../services/subscriptions.service"));
const response_1 = require("../utils/response");
const createSubscription = async (req, res) => {
    try {
        const userId = req.user.userId;
        const subscription = await subscriptionsService.createSubscription(userId, req.body);
        return (0, response_1.sendSuccess)(res, subscription, 'Subscription created successfully', 201);
    }
    catch (error) {
        return (0, response_1.sendError)(res, error.message);
    }
};
exports.createSubscription = createSubscription;
const getMySubscriptions = async (req, res) => {
    try {
        const userId = req.user.userId;
        const subscriptions = await subscriptionsService.getMySubscriptions(userId);
        return (0, response_1.sendSuccess)(res, subscriptions);
    }
    catch (error) {
        return (0, response_1.sendError)(res, error.message);
    }
};
exports.getMySubscriptions = getMySubscriptions;
const getSubscriptionById = async (req, res) => {
    try {
        const userId = req.user.userId;
        const { id } = req.params;
        const subscription = await subscriptionsService.getSubscriptionById(userId, id);
        return (0, response_1.sendSuccess)(res, subscription);
    }
    catch (error) {
        return (0, response_1.sendError)(res, error.message, null, 404);
    }
};
exports.getSubscriptionById = getSubscriptionById;
const renewSubscription = async (req, res) => {
    try {
        const userId = req.user.userId;
        const { id } = req.params;
        const subscription = await subscriptionsService.renewSubscription(userId, id);
        return (0, response_1.sendSuccess)(res, subscription, 'Subscription renewed successfully', 201);
    }
    catch (error) {
        return (0, response_1.sendError)(res, error.message);
    }
};
exports.renewSubscription = renewSubscription;
